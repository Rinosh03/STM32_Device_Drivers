...
#include <string.h>
uint8_t Rxbuff[10];
USART_Struct_T usart2config = { .baudrate = 9600, .oversampling = 8, .parity =
		USART_PARITY_NONE, .stopbits = USART_STOPBIT_1, .usartid = USART2_ID,
		.wordlength = USART_WORDLENGTH_8B, .USARTInstance = USART2 };
GPIO_Pinconfig Pushbutton = { .pin = 0, .mode = GPIO_MODE_INPUT, .ospeed =
		GPIO_OSPEED_LOW, .pupdr = GPIO_NOPUPDR, .otype = GPIO_OTYPE_PP,
		.alternatefunc = 0 };

GPIO_Pinconfig RedLED = { .pin = 14, .mode = GPIO_MODE_OUTPUT, .ospeed =
		GPIO_OSPEED_LOW, .pupdr = GPIO_NOPUPDR, .otype = GPIO_OTYPE_PP,
		.alternatefunc = 0 };
